...
public class UpdateDateSpecification implements Specification<UpdateDate> {
	...
	public Predicate toPredicate(
		...
		switch (period) {
		case LAST_MONTH:
			return cb.between(dataExpression, now.minus(30, ChronoUnit.DAYS), now);

		case LAST_6_MONTHS:
			return cb.between(dataExpression, now.minus(MONTHS_6, ChronoUnit.DAYS), now);

		case LAST_YEAR:
			return cb.between(dataExpression, now.minus(YEAR, ChronoUnit.DAYS), now);
